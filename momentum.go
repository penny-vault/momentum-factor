...
func (s *MomentumFactor) Compute(ctx context.Context, eng *engine.Engine, strategyPortfolio portfolio.Portfolio) error {
	...
	// 2. Fetch 12-month window of monthly close prices for all members.
	priceDF, err := indexUniverse.Window(ctx, portfolio.Months(12), data.MetricClose)
	if err != nil {
	...
	momentum := ret12.AddScalar(1).Div(ret1.AddScalar(1)).AddScalar(-1)
	momentum = momentum.Drop(math.NaN()).Last()
